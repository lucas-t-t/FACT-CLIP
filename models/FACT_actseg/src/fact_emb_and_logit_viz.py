...
"""
Visualise a trained FACT model on the Breakfast dataset with **four** UMAP plots:

1. Raw **visual-branch** embeddings (frame tokens, ≤ --max-per-class).
2. Raw **action-token** embeddings (fixed M tokens, no cap).
3. **Frame-logit rail**  Pᶠ  (class-logits produced directly from frame tokens).
4. **Token-logit rail**  P̃ᵃ (action-token logits broadcast to frames via Λᶠ).

Speed & GPU-friendly tweaks
──────────────────────────
✔  Cap frame tokens (default 200) to keep UMAP fast.  
✔  **GPU path**: if RAPIDS *cuML* is available, PCA + UMAP run entirely on GPU
   (cupy) and only the final 2-D points are copied back to CPU.  
✔  **CPU fallback**: scikit-learn PCA + umap-learn, with the earlier
   “metric” argument bug fixed.  
"""
...
# ───────── collect two frame-logit rails (cap applied) ────────────
def collect_logits(model, loader, cache, n_cls, device, cap):
    ...
    with torch.no_grad():
        for sample in loader:
            ...
            frame_logit  = last_blk.frame_clogit.squeeze(1)[:, :n_cls]   # (T_f , C)
            action_logit = last_blk.action_clogit.squeeze(1)[:, :n_cls]  # (M , C)
            Lambda_f     = last_blk.a2f_attn.squeeze(0)                  # (T_a , M)
            ...
            token_idx   = Lambda_f.argmax(dim=-1)            # (T,)
            token2frame = action_logit[token_idx]            # (T , C)

            for i, cls in enumerate(labels):
                if per_cls.get(cls,0) >= cap: continue
                per_cls[cls] = per_cls.get(cls,0)+1
                rows_f.append(frame_logit[i].unsqueeze(0))   # (1 , C)
                rows_a.append(token2frame[i].unsqueeze(0))
                labs.append(int(cls))

    Pf = torch.cat(rows_f, dim=0)   # (N , C)
    Pa = torch.cat(rows_a, dim=0)
    return Pf, Pa, torch.tensor(labs)

# ─────── dimensionality reduction (GPU-aware, bug-free) ───────────
def umap2d(t: torch.Tensor, n_neighbors: int = 50, min_dist: float = 0.1):
    """
    PCA-50 → UMAP-2.
    • GPU path (cupy + cuML) if available and `t` is on CUDA.
    • CPU fallback (sklearn + umap-learn) otherwise.
    """
    max_comp = min(50, t.shape[1] - 1, t.shape[0] - 1)  # ← NEW
...
if __name__ == "__main__":
    ...
    Pf, Pa, yL   = collect_logits(model, loader, cache,
                                  n_cls, args.device, args.max_per_class)
