...
cfg = setup_cfg(['src/configs/breakfast.yaml'], [])

# Override via command line
cfg = setup_cfg(['src/configs/breakfast.yaml'], ['aux.gpu', '0', 'lr', '0.001'])
```
...
```bash
python -m src.train --cfg src/configs/breakfast.yaml \
    --set aux.gpu 0 \
         lr 0.0001 \
         batch_size 2 \
         use_clip true
...
python -m src.train \
    --cfg src/configs/breakfast.yaml src/configs/clip_settings.yaml
```
...
cfg.merge_from_file('src/configs/breakfast.yaml')
...
```bash
python -m src.configs.generate_havid_configs
```
