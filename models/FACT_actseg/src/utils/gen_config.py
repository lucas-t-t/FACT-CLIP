...
Tips:
    ...
    * Use '--match_mode o2m' if videos often contain many repeated actions.
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Optional explicit path to features folder. Useful when features are shared outside the dataset_path (e.g., HAViD)'
    )
    ...
    # Feature folder: allow override via --features_path (needed for HAViD where features are shared)
    feature_folder = Path(args.features_path) if args.features_path is not None else (dataset_path / 'features')
    ...
    def _read_text_with_fallback(path: Path):
        try:
            ...
            # Fallback for datasets containing non-UTF8 symbols
            with open(path, 'r', encoding='latin-1') as f:
                return f.read()

    for label_file in tqdm(label_files, desc="Analyzing labels"):
        content = _read_text_with_fallback(label_file)
    ...
    # These dataset-specific paths are handled by create_dataset() function
    # We don't add them to the config as they're not part of the default schema
    # Instead, they need to be handled in the dataset creation logic
